  ...
  for (;;) {
    if (!min_weight && !max_weight) {
      ...
      if (current_weight) {
        ...
        lcd.noBlink();
        lcd.setCursor(0, 3);
        lcd.print("WEIGHING: " + String(current_weight) + " g.");
        Total++;
        lcd.setCursor(0, 1);
        ...
        if (current_weight >= min_weight && current_weight <= max_weight) {
          count++;
          Serial.println("Passed: " + String(current_weight) + " g.");
          clearScreen(3);
          lcd.setCursor(7, 3);
          lcd.print("PASSED");
          ...
          countNC++;
          Serial.println("Fail: " + String(current_weight) + " g.");
          clearScreen(3);
          lcd.setCursor(4, 3);
          lcd.print("<< FAILED >>");
          alert();
  ...
  while (min_weight && max_weight && !checkDeviceState) {
    ...
    if (Sensor_CurrentState == 1) {
      ...
      if (readString.length() > 0 && Sensor_PreviousState == true) {
        readString.replace("+", "");
        readString.replace("g", "");
        float current_weight = readString.toFloat();

        if (readString.indexOf("-") && PCS_Cache == current_weight && current_weight > 0) {
          if (PCS_TimerCheck == 0) {
          ...
          PCS_Cache = current_weight;  // รีเซ็ตค่าน้ำหนักที่นำมาเปรียบเทียบ
          PCS_TimerCheck = 0;          // รีเซ็ตเวลาเริ่มต้นการวาง
        }
        ...
        Serial.println(current_weight);  // พิมพ์สตริงที่แปลงจากค่าที่อ่านได้
      }
    } else {
      readString = "";
      PCS_Cache = 0;
